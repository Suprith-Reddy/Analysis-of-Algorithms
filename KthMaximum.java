...
public class KthMaximum {
    ...
    private static int heapSort(int[] input,int k){
        int[] output = new int[input.length];
        for(int len = input.length-1; len>(input.length-1)-k; len--){
            output = buildMaxHeap(input, len);
        }
        return output[(input.length-1)-k+1];
    }
}
